...
- X axis = **arrival time in seconds** (wall clock of each received frame, per-recording epoch): channels tracking the backend's own clock (e.g. `periods_count`) plot as straight lines even when replies arrive in bursts — a fixed 1/rateHz tick per frame bent them, because bursty reception compressed/stretched real time. Paused gaps are stitched out by shifting the epoch on resume (injectable `clockNanos` for tests). The follow edge advances at exactly 1 s/s via a frame-clock loop (`lastX + elapsed since arrival`, extrapolation capped at 0.2 s) — sample-chasing tweens made the scroll speed jitter. Status bar and CSV (`time_s`) use the same axis.
